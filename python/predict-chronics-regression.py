...
def input_fn(df):
  ...
  continuous_cols = {k: tf.constant(df[k].values) for k in df.columns}
...
output_labels = ['Y_HYPERTENSION_DX_DATE_3YR']
# Consts
batch_size = 50
nfolds = 5
#dsplit = 20
...
predicts = data.ix[(data['Y_HYPERTENSION_DX_DATE_3YR']==1)]
#predicts = predicts.append(data.ix[(data['Y_DIABETES_DX_DATE_NEXT_3YR']==1)])
#predicts = predicts.append(data.ix[(data['Y_HYPERLIPIDEMIA_DX_DATE_3YR']==1)])
#predicts = predicts.append(data.ix[(data['Y_CAD_DX_DATE_3YR']==1)])
# Drop chronics from data
data.drop(data[data['Y_HYPERTENSION_DX_DATE_3YR']==1].index, inplace=True)
#data.drop(data[data['Y_DIABETES_DX_DATE_NEXT_3YR']==1].index, inplace=True)
#data.drop(data[data['Y_HYPERLIPIDEMIA_DX_DATE_3YR']==1].index, inplace=True)
#data.drop(data[data['Y_CAD_DX_DATE_3YR']==1].index, inplace=True)
# Add and remove labels
predicts['CHRONIC_DX'] = np.ones(len(predicts))
data['CHRONIC_DX'] = np.zeros(len(data))
# Shuffle the data
#ndim = len(data)
#perm = np.arange(ndim)
...
#labels = label_encoding(data['CHRONIC_DX'],'CHRONIC_DX')
labels = data['CHRONIC_DX'].values.astype(int)
label = tf.constant(df[LABEL_COLUMN].values)
data.drop(output_labels,axis=1,inplace=True)
inputs = data.drop('CHRONIC_DX',axis=1).as_matrix()
...
for j in range(nfolds):
	...
	# Predictions - CHECK WHAT TO DO WITH SINGLE DIMENSION PREDICTIONS
