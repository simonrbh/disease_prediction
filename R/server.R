...
shinyServer(function(input, output) {
    ...
    case_input$CHRONIC_RENAL_DISEASE_DURATION_DEG_ACTIVE <- factor(case_input$CHRONIC_RENAL_DISEASE_DURATION_DEG_ACTIVE, levels = c("N", "Y"))
    case_input$HYPERTENSION_DURATION_DEG_ACTIVE <- factor(case_input$HYPERTENSION_DURATION_DEG_ACTIVE, levels = c("N", "Y"))
    case_input$HYPERLIPIDAEMIA_DURATION_DEG_ACTIVE <- factor(case_input$HYPERLIPIDAEMIA_DURATION_DEG_ACTIVE, levels = c("N", "Y"))
    ...
    case_all_trees[case_all_trees == "Y"] <- 1
    case_all_trees[case_all_trees == "N"] <- 0
    case_all_trees <- as.matrix(case_all_trees, nrow = dim(case_pred$aggregate)[1])
